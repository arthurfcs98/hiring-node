...
describe("StockingAPI", () => {
  ...
  describe("fetchQuote", () => {
    it("should calls get with correct params", async () => {
      ...
      expect(getApiSpy).toHaveBeenCalledWith("/stocks/any/quote");
    });
  ...
  describe("fetchHistory", () => {
    it("should calls get with correct params", async () => {
      await StockingAPI.fetchHistory(
        "any",
        new Date("2021-10-01"),
        new Date("2021-10-20")
      );
      expect(getApiSpy).toHaveBeenCalledWith(
        "/stocks/any/history?from=2021-10-01&to=2021-10-20"
      );
    });
    it("should throws StockNotFound when axios return 404 status code", async () => {
      getApiSpy.mockRejectedValue({
      ...
      const sut = StockingAPI.fetchHistory(
        "any",
        new Date("2021-10-01"),
        new Date("2021-10-20")
      );
  ...
  describe("fetchStockGains", () => {
    it("should calls get with correct params", async () => {
      ...
      expect(getApiSpy).toHaveBeenCalledWith(
        "/stocks/any/gains?purchasedAt=2021-10-01&purchasedAmount=10"
      );
    });
    it("should throws StockNotFound when axios return 404 status code", async () => {
      getApiSpy.mockRejectedValue({
      ...
      const sut = StockingAPI.fetchStockGains(
        "any",
        new Date("2021-10-01"),
        10
      );
